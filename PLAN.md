...
- [x] Retry only rejected local validation responses, with an atomic three-attempt cap. (2026-07-15 11:47 EDT)
  The fake Ollama transport returns malformed PO then valid PO and proves the current batch alone retries; three malformed responses fail without changing the catalog. The immutable source header is restored by discarding the model pre-body block, even when it misspells `msgid` as `msg_id`, avoiding retries for non-translatable metadata churn. Stream/API failures remain fail-fast. A mutation lowering the cap from three requests to two makes the exhaustion regression fail. Curiosity poke: retries must never turn an unavailable service into an unbounded wait.
